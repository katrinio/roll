...
- `rl load` (= `rl stock load`) — забрать одну штуку из запаса и создать новый roll.
...
`features`/`keywords` уже есть в формате `roll.toml`, но команды, которая заполняла бы их после создания roll, пока нет — заполнить их сейчас можно только вручную, отредактировав файл.
