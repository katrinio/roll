...
def count_photo_files(folder: Path) -> int:
    ...
    return sum(1 for path in folder.rglob("*") if path.is_file() and path.suffix.lower() in {".jpg", ".jpeg", ".png", ".heic", ".tif", ".tiff"})
...
def build_archive_tree(archive: Path) -> list[str]:
    ...
    for year_dir in year_dirs:
        ...
        roll_folders = sorted(path for path in year_dir.iterdir() if path.is_dir() and path.name != ".roll")
